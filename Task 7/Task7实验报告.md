...
本次测试只针对 `deepseek-v4-flash` 在指定接口和参数下的一次测评结果，不能直接代表其他模型版本或部署方式。模型输出具有随机性，在不同时间、参数和服务端策略下重复实验，命中数量可能发生变化。

各类探针的样本规模并不相同，因此不同类别之间的通过率不能直接横向比较。garak 检测器反映的是特定攻击模板和判定规则下的结果，不能代替人工安全审查或真实业务环境中的风险评估。

本实验重点检查模型输出层面的风险，没有覆盖模型供应链、训练数据、权限控制、网络隔离和实际工具调用等问题。
...
- `promptInjection.png`：提示注入测评结果
- `dan.png`：DAN 越狱测评结果
- `toxicity.png`：毒性内容测评结果
- `apikey.png`：API Key 泄露测评结果
- `exploitation.png`：利用型攻击测评结果
- `malwaregen.png`：恶意代码生成测评结果
